...
* ​`特典标号`​ 是*特典类别加上编号*
  ...
  | 特典类别 |​ `TYPE`​ | 命名规则 |
  | - | - | - |
  |预告 Preview (Prev)<br />预告下一话内容|Preview<br />Web Preview<br />|编号表示其预告的是第几话的内容, 而不是跟在哪一话后面|
  ...
  - 关于编号
    - 如果某一类特典只有一个, 则不需要标注编号
    - 如果某一类特典有多个, 则必须标注编号

  - 关于空格
    - 如果特典类别只有一个整体单词, 编号与特典类别之间不需要添加空格, 如 Menu01
    - 如果特典类别是多个单词由空格分开, 编号与特典类别之间需要添加空格, 如 Web Preview 01

  - 关于下划线
    - 原则上只允许 Menu 和 NC 特典使用下划线命名, 其余特典应当使用空格

  - 关于括号标注
    - 一般只限对正片标注 OVA/OAD, 或者对同一内容标注不同的版本
    - 不同版本应当使用 `(xx Ver.)` 进行标注
    - 括号前面字段不含空格时, 括号应当紧贴前面字段, 如 13(OVA)、PV01(xx Ver.)
    - 括号前面字段含有空格时, 括号与前面字段之间加入一个空格, 如 Character PV 01 (xx Ver.)
...
* ​`画质级别`​ 是*编码级别和分辨率信息*, 如下所示: 
  ...
  |HEVC Main Profile<br />AVC High Profile<br />AV1 8-bit 任意 Profile|1080p, 720p ...|
...
* ​`语言标记`​
  ...
  优先使用字幕组的表述方式, 在项目看板上或者询问总监获取该信息
...
[VCB-Studio] Ajin <br> ./[Kamigami&VCB-Studio] Ajin [Ma10p_1080p] <br> ./[VCB-Studio] Ajin The Movie [Ma10p_1080p] <br> |  注意其 TV 版两季度官方名称一样 <br> 因此 TV 部分是连续两期合并 <br> 非所有季度都有的字幕组合作, 系列 TAG 上不写
