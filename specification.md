...
1. 整理环节依序包括*分配* → *整理* → *复查*：
   ...
   - **分配**
     ...
     - 新任务会反映在 Trello 上，总监和整理组管理会做通知，可以主动接锅或者被分配
     - 三次元正事优先，确认自己一定有时间再接锅，保证质量同时也不要耗时太久
     - 因为三次元事务无法完成任务时请及时将任务转交给其它成员

   - **整理**：检查*压制成品* → (可选)收集相关音乐和扫图 → 组织目录结构和制种 → 上传*整理成品*

     - 一般每个项目的整理环节只由一名整理成员负责，且未来同一系列的番剧都由同一成员负责
     - 整理完成时整理成员须在整理组通知该消息并在度盘分享链接中附上 ATI 和 MP 截图

   - **复查**：检查*整理成品* → 指导错误修正 → 确认项目成品

     - 每个项目的复查环节由项目总监和至少一名整理组复查成员负责
     - 复查通过时复查成员须在整理组通知该消息并贴出项目成品的链接
...
2. 命名
   ...
   - `TAG` 是*组名*，默认 `VCB-Studio`
     ...
     - 原则上合作组组名前置，具体表示方式参考 Trello 卡片
   - `SERIES` 是*系列共有名*，不标 S1+S2+OVA 等季度标识
   - `SEASON` 是*季度专有名*，不标 S1/S2 等季度标识
     - 系列和季度名原则上只使用基本拉丁字母和半角符号之外的字符，避免使用 `\/:*?"<>|`
   ...
   - `LEVEL` 是*编码级别*，如下所示：
     ...
     --- | ---
     现标准版 HEVC Main 10 Profile | Ma10p_4K_HDR, Ma10p_1080p, Ma10p_720p …
...
4. 目录结构
   ...
   专辑目录 | ~/CDs/`CD` | `CD` 是*专辑名*，命名规范见下一小节 <br> **同一专辑 CD 音质的所有版本都放在同一专辑目录** <br> **Hi-Res 版本单独建立文件夹**
   音频 | 单碟/多碟整轨: <br> \~/CDs/`CD`/`CATALOG`.ext <br> 单碟分轨: <br> \~/CDs/`CD`/`TRNUM`. `TRTITLE`.ext <br> 多碟分轨: <br> \~/CDs/`CD`/`CATALOG`/`TRNUM`. `TRTITLE`.ext | `CATALOG` 是品番编号，可以从 [VGMDB](https://vgmdb.net)、官网、销售网站找到 <br> `TRNUM` 是编号，`TRTITLE` 是曲名 <br> `TRNUM` 与 `TRTITLE` 之间用 点+空格 分开 <br> foobar2000 命名模板：`%tracknumber%. %title%`
   CUE 分轨信息 | ~/CDs/CD/`CATALOG`.cue  | 随对应音频放在同一目录
   LOG 抓轨日志 | ~/CDs/CD/`CATALOG`.log  | 随对应音频放在同一目录
